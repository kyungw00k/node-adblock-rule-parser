...
const idleHandler = require('../idle-handler')
const FilterNormalizer = require('./filter-normalizer')
const Filter = require('./filter').Filter
const SelectorFilter = require('./filter').SelectorFilter
const FilterSet = require('./filter-set').FilterSet
const BlockingFilterSet = require('./filter-set').BlockingFilterSet
const determineUserLanguage = require('../determine-user-language')
...
MyFilters.prototype.fetch_and_update = function(id, isNewList) {
  ...
  var ajaxRequest = {
    ...
    headers: {
      "Accept": "text/plain",
      "X-Client-ID": "AdBlock/2.46" /*+ STATS.version*/,
      "If-Modified-Since": this._subscriptions[id].last_modified || new Date().getTime()
    },
  ...
  /*$.ajax*/require('najax')(ajaxRequest);
}
...
MyFilters.prototype._updateSubscriptionText = function(id, text, xhr) {
  ...
  // which modified the checks in filter-normalizer.js.
  if (xhr) {
